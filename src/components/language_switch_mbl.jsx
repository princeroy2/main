...
const Lang_Btn_list4_mbl = () => {
  ...
  const handleLanguageChange = (languageCode, languageText) => {
    if (isScriptLoaded && window.google && window.google.translate) {
      ...
      if (selectElement) {
        ...
        // If Google Translate is not ready, retry after a delay
        setTimeout(() => handleLanguageChange(languageCode, languageText), 500);
        console.log('Google Translate select element not found, retrying...');
      }
    } else {
      console.log('Google Translate is not available, retrying...');
      // If the script isn't loaded yet, retry after a short delay
      setTimeout(() => handleLanguageChange(languageCode, languageText), 500);
    }
  };
  ...
  return (
    <div className="absolute top-5 right-0">
      ...
      <div
        onClick={toggleList}
        className="text-white gap-1 flex-row  px-2  cursor-pointer flex items-center notranslate skiptranslate"
        style={{ zIndex: 100 }}
      >
      ...
      {isOpen && (
        <div
          className="text-[#2f3c42] cursor-pointer mt-2 absolute p-1 right-3 bg-white rounded-md shadow-lg notranslate skiptranslate"
        >
